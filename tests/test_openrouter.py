#!/usr/bin/env python3
"""
Standalone OpenRouter API test — run this before using OpenRouter in the cluster.
Tests: connectivity, auth, model validity, JSON compliance, speed.

OpenRouter requires extra headers: HTTP-Referer and X-Title.
Free models have a ":free" suffix in their ID.
...
# ── Config ────────────────────────────────────────────────────────────────────
BASE_URL = os.getenv("PROVIDER_OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
MODEL    = os.getenv("PROVIDER_OPENROUTER_MODEL", "meta-llama/llama-3.1-8b-instruct:free")
RAW_KEYS = os.getenv("PROVIDER_OPENROUTER_KEYS", "")
KEYS     = [k.strip() for k in RAW_KEYS.split(",") if k.strip()]

# OpenRouter-specific headers required on every request
OR_HEADERS = {
...
async def test_config():
    ...
    for i, k in enumerate(KEYS):
        info(f"  key[{i}] : {k[:12]}...{k[-4:]}")
    if not KEYS:
        err("No keys found — set PROVIDER_OPENROUTER_KEYS in .env")
        err("Get a free key: https://openrouter.ai/keys")
        return False
    if ":free" in MODEL:
        info(f"Note: '{MODEL}' is a free model (no credits needed)")
    ok("Config loaded")
    return True
...
async def test_connectivity():
    ...
    try:
        ...
        ok(f"openrouter.ai reachable (HTTP {r.status_code})")
        return True
    except Exception as e:
        err(f"Cannot reach openrouter.ai: {e}")
        return False
...
async def test_auth(key: str, idx: int):
    ...
    if result["status"] == 402:
        err("402 Payment Required — model requires credits, try a ':free' model")
        info("Free models: meta-llama/llama-3.1-8b-instruct:free, google/gemma-2-9b-it:free")
        return False
    if result["status"] == 403:
        err("403 Forbidden — key may be expired or rate limited")
        return False
    if result["status"] == 400:
        err(f"400 Bad Request — likely wrong model name: '{MODEL}'")
        info(f"Response: {result['body'][:400]}")
        info("Browse free models at: https://openrouter.ai/models?q=:free")
        return False
...
async def test_model_list(key: str):
    hdr("4. Available models (free tier)")
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            r = await client.get(
                f"{BASE_URL}/models",
                headers={"Authorization": f"Bearer {key}", **OR_HEADERS},
            )
        ...
        ok(f"{len(all_models)} total models, {len(free_models)} free (:free suffix):")
        for m in free_models[:20]:
            marker = " ◄ CURRENT" if m == MODEL else ""
            info(f"    {m}{marker}")
        if len(free_models) > 20:
            info(f"    ... and {len(free_models) - 20} more free models")
        if MODEL not in all_models:
            err(f"'{MODEL}' not found — check https://openrouter.ai/models")
    except Exception as e:
...
async def main():
    ...
    if not await test_config():
        return {"provider": "OpenRouter", "ok": False, "latency": None, "model": MODEL}
    ...
    print(f"\n{YLW}── Summary{RST}")
    if auth_ok:
        ok("OpenRouter is working — safe to use in the cluster")
        info(f"Recommended .env setting: PROVIDER_OPENROUTER_MODEL={MODEL}")
        info("Note: cluster will automatically include HTTP-Referer and X-Title headers")
    else:
        err("OpenRouter is NOT working — check your key at https://openrouter.ai/keys")
    print()

    return {"provider": "OpenRouter", "ok": auth_ok, "latency": avg_latency, "model": MODEL}
